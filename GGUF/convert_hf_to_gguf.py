...
# Monkeypatch missing Enum attributes in older gguf versions to ensure compatibility with latest llama.cpp scripts
import enum
original_getattr = enum.EnumMeta.__getattr__
def patched_getattr(cls, name):
    if name == "GEMMA4_ASSISTANT":
        return cls.GEMMA4
    return original_getattr(cls, name)
enum.EnumMeta.__getattr__ = patched_getattr
