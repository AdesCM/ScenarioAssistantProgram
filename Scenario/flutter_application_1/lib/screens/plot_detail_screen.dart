...
class PlotDetailScreen extends StatefulWidget {
  static const routeName = '/plot-detail';
...
class _PlotDetailScreenState extends State<PlotDetailScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (_isInit) {
      ...
      _plot = universe.plots.firstWhere((plot) => plot.id == plotId);
      _currentImageUrl = _plot.imageUrl;

      _plot.details.forEach((key, value) {
  ...
  void _saveForm() {
    ...
    final updatedPlot = Plot(
      id: _plot.id,
      name: updatedDetails['이름'] ?? _plot.name,
    ...
    Provider.of<UniverseProvider>(context, listen: false)
        .updatePlot(_universeId, updatedPlot);
